...
import shutil
from utils.run_blink import train_once
...
def add_args():
    ...
    parser.add_argument('--use-best-lr', action='store_true', default=False, help='whether to use the best learning rate')
...
def main(dataset='cora', model='gcn', dense=False, args=None, variant='hard'):
    ...
    if args.use_best_lr:
        with open('./best_lr.json', 'r') as f:
            best_lr = json.load(f)

    result = nested_defaultdict()
    if 'hpgr' in args.methods:
        method = 'modified'
        for dataset in args.datasets:
            for i in range(len(args.tau)):
                ...
                for model in args.gnn_models:
                    for eps in args.eps_list:
                        for variant in args.variant:
                            if variant == 'soft' and model == 'gat':
                                print('skip gat soft!')
                                continue
                            for sbm_ratio in args.sbm_ratio_list:
                                for degree_ratio in args.degree_ratio_list:
                                    if args.use_best_lr and dataset in best_lr.keys() and model in best_lr[dataset].keys():
                                        train_args['lr'] = best_lr[dataset][model]['hard' if variant == 'hard' else 'soft']
                                    else:
                                        train_args['lr'] = 0.001
                                        print('not use best lr!')
                                    ...
                                    for graph_seed in args.graph_seeds:
                                        acc = train_once(data=data, method=method, blink_args=blink_args, 
                                                        model_args=model_args, model=model, variant=variant, 
                                                        train_args=train_args, graph_seed=graph_seed, 
                                                        gnn_seeds=args.gnn_seeds, device=args.device, use_soft=use_soft
                                                    )
                                        acc_list.append(acc)
                                    result[method][dataset][tau][model][eps][variant][sbm_ratio][degree_ratio] = [np.mean(acc_list), np.var(acc_list)]
                        release_cuda_memory()
                            ...
                            for delta in args.delta_list:
                                ...
                                for graph_seed in args.graph_seeds:
                                    acc = train_once(data=data, method=method, blink_args=blink_args, 
                                                    model_args=model_args, model=model, variant=variant, 
                                                    train_args=train_args, graph_seed=graph_seed, 
                                                    gnn_seeds=args.gnn_seeds, device=args.device
                                                )
                                    acc_list.append(acc)
                                result[method][dataset][model][eps][variant][delta] = [np.mean(acc_list), np.var(acc_list)]
                        release_cuda_memory()
                        result_ckp = defaultdict_to_dict(result)
                        with open(osp.join(save_root, 'result_ckp.json'), 'w') as f:
                            json.dump(result_ckp, f, indent=4)
